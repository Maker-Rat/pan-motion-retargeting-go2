...
from data_preprocess.Lafan1_and_dog.extract import get_lafan1_example
...
def main():
    ...
    ori_name = [name[:-4] for name in os.listdir(bvh_dir)]
    files = [os.path.join(bvh_dir, name) for name in os.listdir(bvh_dir)]
    num = 0

    for file in files:
        print("retargeting the human motion %s to dog skeleton" % str(file))
        X, Q, Pos, V, parents, yrot, offsets, offsets_withend = get_lafan1_example(file)
        ...
        src_anim = Anim(Q_src.squeeze(), Pos_src.squeeze(),
                       std_hum_anim.offsets, std_hum_anim.parents, std_hum_anim.bones)
        ...
        data = (data - humdataset.mean[np.newaxis, np.newaxis, ...]) / humdataset.std[np.newaxis, np.newaxis, ...]

        vel_dim = 4
        input_h_encoder = torch.Tensor(data[..., :args.hum_njoints * 4 + vel_dim]).transpose(1, 2).to(device)
        input_d_encoder = torch.zeros(data.shape[:-1] + (args.dog_njoints * 4 + vel_dim,)
                                      ).transpose(1, 2).to(device) # Placeholder, meaningless

        input_h_encoder = input_h_encoder[..., :args.time_size]
        input_d_encoder = input_d_encoder[..., :args.time_size] # Placeholder, meaningless

        input_h_encoder = (input_h_encoder, offsets, offsets_withend)
        input_d_encoder = (input_d_encoder, offsets[..., :(args.dog_njoints-1)*3], offsets_withend) # Placeholder, meaningless

        model.set_input([input_h_encoder, input_d_encoder])
        model.forward()
        src, retar = model.motion_denorm[0], model.fake_retar_denorm[0]

        retar_q = qnorm(retar[..., :-vel_dim].reshape(-1, args.dog_njoints, 4))
        ...
        retar_anim = Anim(retar_q_np, pos, std_dog_anim.offsets, std_dog_anim.parents, std_dog_anim.bones)

        if not os.path.exists(os.path.join(args.save_dir, 'demo/hum2dog')):
            os.mkdir(os.path.join(args.save_dir, 'demo/hum2dog'))
        bvh_name = os.path.join(os.path.join(args.save_dir, 'demo/hum2dog'), ori_name[num]+'_retar.bvh')
        save_bvh(bvh_name, retar_anim, frametime=1 / 30, order='zyx', with_end=False,
                 names=retar_anim.bones, end_offset=dog_end_offsets)

        remove_foot_sliding_humdog(bvh_name, bvh_name, end_site=True)

        bvh_src = os.path.join(os.path.join(args.save_dir, 'demo/hum2dog'), ori_name[num] + '_source.bvh')
        save_bvh(bvh_src, src_anim, frametime=1 / 30, order='zyx',
                 with_end=False, names=src_anim.bones, end_offset=hum_end_offsets)
